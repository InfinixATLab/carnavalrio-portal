            ...
            {article.title && (
                <>
                    ...
                    <main className="mx-4 mb-8 my-4 md:mx-auto md:max-w-[50%]">
                        <div className="my-4">
                            <p className="w-1/2 text-[#6e3a62ff] font-bold text-sm">{article.school?.name}</p>
                        </div>
                        <div>
                            <h1 className="font-bold w-full min-h-max border-black">{article.title}</h1>
                            <p className="text-gray-500 text-xs my-1">Por {article.author_name} {article.author.surname} | <span className="text-[10px]">{formatarDataHora(article.updated_at)}</span></p>
                            <hr className="my-2" />
                        </div>
                        <div>
                            <div
                        ...
                        <div className="max-w-full">
                            <div className="text-sm leading-6 w-full my-4 break-words prose [&>p]:mb-4 [&>div]:mb-4 [&>h1]:mb-1 [&>h1>strong]:text-1xl [&>h2]:mb-1 [&>h2>strong]:text-lg [&>h3]:mb-1 [&>h3>strong]:text-md" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(article.body) }} />
                        </div>
                    </main>
                    <Footer />
                </>
